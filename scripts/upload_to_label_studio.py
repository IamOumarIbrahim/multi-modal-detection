...
def main():
    ...
    xml_config = Path("annotations/label_studio_video_config.xml").read_text(encoding="utf-8")
    payload = {
        "title": "MMSAR Desert Positive Video Tracking (24 FPS)",
        "label_config": xml_config,
        "description": "Tracking Person_Detected on 5 Desert Positive RGB videos (native 24 FPS)",
    }
    session.patch(f"{BASE_URL}/api/projects/{project_id}", headers=headers, json=payload)
    print("Updated Project 2 label_config to 24.0 FPS.")
    ...
    # Upload 24 FPS videos
    video_dir = Path("data/processed/label_studio_videos")
    video_files = sorted(video_dir.glob("*.mp4"))
    print(f"Found {len(video_files)} 24 FPS video files to import into project {project_id}:")
    ...
    for v in video_files:
        with open(v, "rb") as f:
            ...
            print(f"  Import {v.name}: status={up_resp.status_code}, result={up_resp.json().get('task_count')}")
    ...
    for t in tasks:
        print(f"  Task {t['id']}: {t['data']}")
    print(f"\nDirect Annotation URL: {BASE_URL}/projects/{project_id}")
