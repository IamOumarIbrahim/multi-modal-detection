...
% (Scaled up by 10%, extended to the left to align with left bus line)
% =========================================================================

% 1. RGB video prompt (Text File Icon - 10% larger, extended left)
\node[iconbox] (prompt_rgb) at (-1.3, 5.7) {
...
\node[below=2pt of prompt_rgb, font=\fontsize{5.3pt}{6.4pt}\selectfont\bfseries, text=black] (sub_p1) {Text Prompt};

% 2. Video Generator LLM (Gemini Icon - 10% larger)
\node[iconbox] (gemini_rgb) at (1.6, 5.7) {
    \includegraphics[height=0.76cm]{figures/gemini_logo.png}
};
\node[above=4pt of gemini_rgb, ttl, text=purple!80!black] (lbl_g1) {Video Generator LLM};
\node[below=2pt of gemini_rgb, font=\fontsize{5.3pt}{6.4pt}\selectfont\bfseries, text=black] (sub_g1) {Gemini 3.8 Flash};

\draw[bluearrow] (prompt_rgb) -- (gemini_rgb);

% 3. 16:9 RGB Videos (Desert & Forest with Striped Bottom - 10% larger)
\node[videobox] (rgb_videos) at (4.8, 5.7) {
    ...
    {\tiny\bfseries Desert RGB} & {\tiny\bfseries Forest RGB}
    \end{tabular}
};
\node[above=4pt of rgb_videos, ttl, text=black] (lbl_v1) {16:9 RGB Videos};
...
% 4. Configure to Thermal (Text File Icon - 10% larger)
\node[iconbox] (prompt_th) at (8.0, 5.7) {
    \includegraphics[height=0.76cm]{figures/icon_lorem_ipsum.png}
};
\node[above=4pt of prompt_th, ttl, text=orange!80!black] (lbl_p2) {Configure to Thermal};
\node[below=2pt of prompt_th, font=\fontsize{5.3pt}{6.4pt}\selectfont\bfseries, text=black] (sub_p2) {Thermal Prompt};

\draw[orangearrow] (rgb_videos) -- (prompt_th);

% 5. Video Generator LLM (Gemini Icon - Modality Transfer - 10% larger)
\node[iconbox] (gemini_th) at (11.0, 5.7) {
    \includegraphics[height=0.76cm]{figures/gemini_logo.png}
};
\node[above=4pt of gemini_th, ttl, text=purple!80!black] (lbl_g2) {Video Generator LLM};
\node[below=2pt of gemini_th, font=\fontsize{5.3pt}{6.4pt}\selectfont\bfseries, text=black] (sub_g2) {Modality Transfer};

\draw[orangearrow] (prompt_th) -- (gemini_th);

% 6. 16:9 Thermal Videos (10% larger)
\node[videobox] (th_videos) at (14.5, 5.7) {
    \begin{tabular}{@{}c@{\hspace{3pt}}c@{}}
    \includegraphics[width=1.12cm]{figures/desert_th_video.png} &
    \includegraphics[width=1.12cm]{figures/forest_th_video.png} \\
    {\tiny\bfseries Desert TIR} & {\tiny\bfseries Forest TIR}
    \end{tabular}
};
...
% 7. Crop 640x640 (10% larger)
\node[videobox] (crops) at (18.5, 5.7) {
    \begin{tabular}{@{}c@{\hspace{3pt}}c@{}}
...
% 8. Label Studio Video Annotation (10% larger)
\node[iconbox] (label_studio) at (21.6, 5.7) {
...
% 9. Video to Frame Full-Rate 24 Hz Extraction (Clean single-line title)
\node[stepbox, text width=1.95cm, inner xsep=3pt, minimum height=1.15cm] (v2f) at (24.2, 5.7) {
...
% 10. Annotated Frames (16:9 Clean Frames with Accurate Person Bounding Boxes)
\node[framebox] (annotated_frames) at (27.0, 5.7) {
    \begin{tabular}{@{}c@{\hspace{3pt}}c@{}}
    \includegraphics[width=0.92cm]{figures/desert_rgb_annotated.png} &
    \includegraphics[width=0.92cm]{figures/desert_th_annotated.png} \\
    {\tiny\bfseries RGB Frame} & {\tiny\bfseries Thermal Frame}
    \end{tabular}
};
...
% Benchmark Corpus & Episodic Splitting Card (Generous padding, perfectly balanced)
\node[draw=purple!70, fill=purple!6, rounded corners=4pt, thick, font=\fontsize{5.8pt}{7.0pt}\selectfont, text=black, inner ysep=4pt, inner xsep=10pt, align=center] (dataset_banner) at (13.5, 3.6) {
